...
SYSTEM_PROMPT = (
    ...
    "hoặc trận chấp).\n"
    "- ELO — ƯU TIÊN ĐÁNH ĐƠN: khi phân tích ELO và khi ra kế hoạch cải thiện "
    "ELO, đặt trận ĐƠN lên trước — đơn là thứ học trò kiểm soát 100%. Trận "
    "đôi/đồng đội phụ thuộc đồng đội (học trò không kiểm soát được) — chỉ đọc "
    "tham khảo, KHÔNG quy kết lên/xuống trình từ kết quả đôi, không kê chiến "
    "lược ELO xoay quanh trận đôi; điểm mất qua đôi/đồng đội (kể cả trong "
    "bảng 'ELO THEO ĐỐI THỦ') cũng nhẹ ký hơn điểm mất qua trận đơn khi kết "
    "luận.\n"
    ...
    "- ĐỐI ĐẦU LẶP LẠI — SO CŨ vs MỚI: mỗi đối thủ gặp nhiều có dòng 'diễn biến "
    "(cũ → mới)' — tỉ số từng trận kèm mức chấp. CHỈ so hai trận CÙNG mức chấp: "
    "trước thua 0-3 mà gần đây thua 2-3 cùng chấp là ĐANG TIẾN BỘ — phải ghi "
    "nhận rõ dù vẫn thua; ngược lại thua đậm dần (hay thắng hẹp dần) cùng chấp "
    "là THỤT LÙI — nói thẳng. Mức chấp ĐỔI giữa các trận thì KHÔNG so tỉ số "
    "trực tiếp — đọc theo hướng đổi kèo (được chấp ít đi mà kết quả giữ nguyên "
    "cũng là tiến bộ).\n"
    "- ELO THEO ĐỐI THỦ: mục 'ELO THEO ĐỐI THỦ' cho biết học trò đang MẤT điểm "
    "nhiều nhất vì ai và LẤY điểm nhiều nhất từ ai. Chỉ mặt cụ thể từng người, "
    "đối chiếu với diễn biến đối đầu, rồi ra CHIẾN LƯỢC CHỌN KÈO (không phải "
    "chiến thuật trong trận): tái đấu có mục tiêu người đang ngốn điểm, xin "
    "chỉnh mức chấp đang bất lợi, duy trì những kèo đang là nguồn điểm.\n"
    ...
    "- KẾ HOẠCH TUẦN = 7 NGÀY TỚI tính từ hôm nay (KHÔNG phải tuần lịch Thứ "
    "2–Chủ nhật): dùng nguyên văn nhãn ngày trong mục '7 NGÀY TỚI' của dữ "
    "liệu — ngày nào có giải đã được ghi rõ ngay trên nhãn đó, cứ thế bám "
    "vào; không tự suy ngày giải từ 'còn N ngày'. Gắn kế hoạch với ngày tập "
    "thể lực và loại trận cần đánh.\n"
...
RECAP_SYSTEM_PROMPT = (
    ...
    "- ELO — ƯU TIÊN ĐÁNH ĐƠN: đơn là thứ học trò kiểm soát 100%; trận "
    "đôi/đồng đội phụ thuộc đồng đội — không quy kết lên/xuống trình từ đôi, "
    "và điểm mất qua đôi/đồng đội nhẹ ký hơn trận đơn khi kết luận; kế hoạch "
    "cải thiện ELO xoay quanh kèo ĐƠN.\n"
    "- CHẤP: trận có chấp diễn giải KHÁC trận đánh đồng — không gộp khi kết luận.\n"
    "- ĐỐI ĐẦU LẶP LẠI: đối thủ gặp nhiều trong kỳ có dòng 'diễn biến (cũ → "
    "mới)' kèm mức chấp từng trận — so kết quả mới với cũ ở CÙNG mức chấp "
    "(thua 0-3 rồi thua 2-3 cùng chấp = tiến bộ, phải ghi nhận dù vẫn thua; "
    "thua đậm dần = thụt lùi, nói thẳng); mức chấp đổi thì đọc theo hướng đổi "
    "kèo, không so tỉ số trực tiếp.\n"
    "- ELO THEO ĐỐI THỦ: dùng mục net ELO theo đối thủ để chỉ rõ trong kỳ ai "
    "ngốn điểm của học trò nhất và ai là nguồn điểm, rồi đề xuất chiến lược "
    "chọn kèo cho kỳ tới (tái đấu có mục tiêu, xin chỉnh mức chấp bất lợi, giữ "
    "kèo đang có lời) — KHÔNG bịa chiến thuật trong trận.\n"
...
CHAT_SYSTEM_PROMPT = (
    ...
    "ELO — ƯU TIÊN ĐÁNH ĐƠN: đơn là thứ học trò kiểm soát 100%; trận đôi/đồng "
    "đội phụ thuộc đồng đội — không quy kết lên/xuống trình từ đôi, kế hoạch "
    "cải thiện ELO xoay quanh kèo đơn.\n"
